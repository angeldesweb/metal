<script>
    import PrintOutbound from '../../Organism/PrintOutbound.svelte';
    ...
    const print =  async (e) => {
        try {
            inAction = true;
            let {out} = await getOutbound(e.target.id);
            date = out.date.toDate().toLocaleDateString('es-ES');
...
<Datatable style="padding:1em;" {settings} {data} bind:dataRows={rows}>
    <thead>
        <th style="padding:1em;" data-key="date">ID</th>
...
<div class="modal">
    <div class="modal-box w-12/12 max-w-5xl">
        <PrintOutbound title="Salida de material" {date}>
            <ul slot="materials">
